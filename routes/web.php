...
use App\Http\Controllers\LessCreditNoteController;
...
Route::middleware(['auth', 'custom.auth'])->group(function () {
    ...
    Route::prefix('credit_note')->group(function () {
        Route::middleware(['role:Super Admin|Admin|Sales User|Sales Admin'])->get('/', [App\Http\Controllers\CreditNoteController::class, 'index'])->name('credit_note.index');
        ...
        Route::prefix('less_credit_note')->group(function () {
        Route::middleware(['role:Super Admin|Admin|Sales User|Sales Admin'])->get('/', [LessCreditNoteController::class, 'index'])->name('less_credit_note.index');
        Route::middleware(['role:Super Admin|Admin|Sales User|Sales Admin'])->get('/create', [LessCreditNoteController::class, 'create'])->name('less_credit_note.create');
        Route::middleware(['role:Super Admin|Admin|Sales User|Sales Admin'])->post('/store', [LessCreditNoteController::class, 'store'])->name('less_credit_note.store');
    });
    ...
    Route::middleware(['role:Super Admin|Admin|Sales User'])->prefix('distributor_invoices')->group(function () {
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/', [DistributorInvoiceController::class, 'all'])->name('distributor_invoices.index');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/new', [DistributorInvoiceController::class, 'new'])->name('distributor_invoices.create');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->post('/new', [DistributorInvoiceController::class, 'store'])->name('distributor_invoices.store');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->post('/item/store', [DistributorInvoiceController::class, 'storeItem'])->name('distributor_invoices.item.store');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->post('/item/delete', [DistributorInvoiceController::class, 'deleteItem'])->name('distributor_invoices.item.delete');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/get/items/table', [DistributorInvoiceController::class, 'itemsTable'])->name('distributor_invoices.item.table');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/{invoice_id}/preview', [DistributorInvoiceController::class, 'preview'])->name('distributor_invoices.preview');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/{invoice_id}/print', [DistributorInvoiceController::class, 'print'])->name('distributor_invoices.print');
        Route::middleware(['role:Super Admin|Admin|Sales Admin'])->get('/{invoice_id}/cancel', [DistributorInvoiceController::class, 'cancel'])->name('distributor_invoices.cancel');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/get/data', [DistributorInvoiceController::class, 'getData'])->name('distributor_invoices.get.data');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/get/total', [DistributorInvoiceController::class, 'getInvoiceTotal'])->name('distributor_invoices.get.total');
        Route::middleware(['role:Super Admin|Admin|Sales User'])->get('/get/invoice_no', [DistributorInvoiceController::class, 'generateInvoiceNumber'])->name('distributor_invoices.get.number');
    });

    Route::middleware(['role:Super Admin|Admin|Warehouse User|Sales Admin'])->prefix('distributor_deliveryorders')->group(function () {
        Route::middleware(['role:Super Admin|Admin|Warehouse User|Sales Admin'])->get('/', [DistributorDeliveryOrderController::class, 'index'])->name('distributor_deliveryorders.index');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/new', [DistributorDeliveryOrderController::class, 'create'])->name('distributor_deliveryorders.create');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->post('/new', [DistributorDeliveryOrderController::class, 'store'])->name('distributor_deliveryorders.store');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/{delivery_order}/view', [DistributorDeliveryOrderController::class, 'view'])->name('distributor_deliveryorders.view');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/get-items', [DistributorDeliveryOrderController::class, 'getInvoiceItems'])->name('distributor_deliveryorders.getInvoiceItems');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/{delivery_order}/issue_delivery_order', [DistributorDeliveryOrderController::class, 'issueIndex'])->name('distributor_deliveryorders.issueIndex');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->post('/{delivery_order}/issue_delivery_order', [DistributorDeliveryOrderController::class, 'issueStore'])->name('distributor_deliveryorders.issueStore');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/{delivery_order}/get', [DistributorDeliveryOrderController::class, 'getById'])->name('distributor_deliveryorders.getById');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/{delivery_order_id}/print', [DistributorDeliveryOrderController::class, 'print'])->name('distributor_deliveryorders.print');
        Route::middleware(['role:Super Admin|Admin|Sales Admin'])->get('/{delivery_order_id}/cancel', [DistributorDeliveryOrderController::class, 'cancel'])->name('distributor_deliveryorders.cancel');
    });
    Route::middleware(['role:Super Admin|Admin|Warehouse User|Sale User|Sales Admin'])->prefix('distributor_balanceOrder')->group(function () {
        Route::middleware(['role:Super Admin|Admin|Warehouse User|Sale User|Sales Admin'])->get('BalanceOrder', [DistributorBalanceOrderController::class, 'index'])->name('distributor_balanceOrder.index');
        Route::middleware(['role:Super Admin|Admin|Warehouse User|Sale User|Sales Admin'])->get('{balance_order}/view', [App\Http\Controllers\DistributorBalanceOrderController::class, 'view'])->name('distributor_balanceOrder.view');
        Route::middleware(['role:Super Admin|Admin|Sales User|Sales Admin'])->get('{balance_order}/delivery-order-create', [App\Http\Controllers\DistributorBalanceOrderController::class, 'deliveryOrderCreateIndex'])->name('distributor_balanceOrder.delivery_order_create_index');
        Route::middleware(['role:Super Admin|Admin|Sale User|Sales Admin'])->post('{balance_order}/delivery-order-create', [App\Http\Controllers\DistributorBalanceOrderController::class, 'deliveryOrderCreate'])->name('distributor_balanceOrder.delivery_order_create');
        Route::middleware(['role:Super Admin|Admin|Warehouse User'])->get('/{balance_order_id}/print', [App\Http\Controllers\DistributorBalanceOrderController::class, 'print'])->name('distributor_balanceOrder.print');
    });

    Route::prefix('organization')->group(function () {
        Route::middleware(['role:Super Admin|Admin|Master Data Entry|Master Data Editor|Sales User|Sales Admin|Executive User'])->get('organization', [OrganizationController::class, 'index'])->name('organization.index');
        Route::middleware(['role:Super Admin|Admin|Master Data Entry|Sales Admin'])->get('/create', [App\Http\Controllers\OrganizationController::class, 'create'])->name('organization.create');
        Route::middleware(['role:Super Admin|Admin|Master Data Entry|Sales Admin'])->post('/create', [App\Http\Controllers\OrganizationController::class, 'store'])->name('organization.store');
        Route::middleware(['role:Super Admin|Admin|Sales User|Warehouse User|Executive User'])->get('/get/data', [OrganizationController::class, 'getData'])->name('organization.get.data');
    });     
