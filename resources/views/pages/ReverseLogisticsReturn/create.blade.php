...
<div class="content-wrapper">
    <div class="row">
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="title">Reverse Logistics Return Entry</h4>
                    <br>
                    <form id="customerReturn" method="POST">
                        @csrf
                        <div class="row">
                            <div class="form-group col-md-2">
                                <label>Customer Code</label>
                                <input type="text" class="form-control" value="{{ $customer_data->customer_code }}" id="customer_code"
                        ...
                        <div class="row">
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control invoice_input" name="invoice_number" id="invoice_number"
                                    ...
                                    @foreach ($reverse_invoices as $row)
                            ...
                            <div class="form-group col-md-3">
                                <label>DO No</label>
                                <input type="text" class="form-control" value="" id="delivery_order_2"
                                name="delivery_order_2" placeholder="DElivery Order" disabled>
                                <select class="form-control invoice_input" name="delivery_order" id="delivery_order"
                                    ...
                                    @foreach ($reverse_delivery_orders as $row)
...
<script>
    ...
    let customerId = "{{$customer_data ? $customer_data->id : null}}";
    let invoiceDate = "{{ $invoice_date }}";
    let invoiceNumber = "{{ $invoice_number }}"
    let deliveryOrderId = "{{ $delivery_order_no }}"
    let reverse_invoices =  <?php echo json_encode($reverse_invoices); ?>;

    $("#customer_id").change(function() {
        customerId = $(this).val();
        console.log("CUSTOMER ON CHANGE")
        window.location.href =
            `/reverse_returns/new?customer_id=${customerId}&invoice_date=${invoiceDate}&invoice_number=${invoiceNumber}&delivery_order_no=${deliveryOrderId}`
    });
    ...
    $("#invoice_number").change(function() {
        ...
        const invoice = reverse_invoices?.find(row => row?.invoice_number ==invoiceNumber)
        console.log(invoice)

        $('#delivery_order_2').val(invoice?.delivery_order?.delivery_order_no)
